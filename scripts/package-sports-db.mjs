 ...
 * Also writes gzip chunks for browser CDN (published flat on GitHub Release assets).
 */
...
import { buildSportsDbGzipCdn, CDN_CHUNKED_FILES } from "./sports-db-gzip-chunks.mjs";
...
async function main() {
  ...
  console.log("\n→ Building sportverse-cdn gzip chunks for browser CDN…");
  buildSportsDbGzipCdn(dataDir, cdnDir, { files: CDN_CHUNKED_FILES });
